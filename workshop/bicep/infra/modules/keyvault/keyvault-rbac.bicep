...
// RBAC: Grant deploying user Key Vault Secrets User role
resource deployerSecretsUserRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = {
  scope: keyVault
  name: guid(subscription().id, resourceGroup().id, keyVault.name, keyVaultSecretsUserRoleId, deployer().objectId)
  properties: {
    roleDefinitionId: resourceId('Microsoft.Authorization/roleDefinitions', keyVaultSecretsUserRoleId)

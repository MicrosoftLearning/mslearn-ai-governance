...
@description('Deploy a dedicated Azure Language service for PII processing. When false, PII processing uses the first AI Foundry AIServices resource.')
param deployDedicatedLanguageService bool = false
...
var resolvedLanguageServiceName = !empty(languageServiceName) ? languageServiceName : '${abbrs.cognitiveServicesAccounts}language-${resourceToken}'
var dedicatedLanguageServiceUrl = 'https://${resolvedLanguageServiceName}.cognitiveservices.azure.com'
var primaryFoundryPiiServiceUrl = 'https://${primaryFoundryName}.cognitiveservices.azure.com'
var piiServiceUrl = deployDedicatedLanguageService ? dedicatedLanguageServiceUrl : primaryFoundryPiiServiceUrl
...
module languageService 'modules/ai/cognitiveservices.bicep' = if (deployDedicatedLanguageService) {
  name: 'ai-language-service'
  scope: resourceGroup
  params: {
    name: resolvedLanguageServiceName
    location: location
    tags: tags
    kind: 'TextAnalytics'
    managedIdentityName: apimManagedIdentity.outputs.managedIdentityName
...
module apim './modules/apim/apim.bicep' = {
  ...
  params: {
    ...
    aiLanguageServiceUrl: piiServiceUrl
